...
class DatesBetweenSpec extends AnyFlatSpec with Matchers {
  ...
  "Given two dates (start exclusive, end exclusive), datesBetween" should "return all dates between except start and end" in {
    val start = daysOfMarch.head
    val end = daysOfMarch.last
  ...
  "Given a LocalDateTime and a LocalDate (start exclusive exclusive), datesBetween" should "return all dates between except the first two" in {
  ...
  "Given a two ZonedDateTimes (start exclusive) with an implicit conversion function, datesBetween" should "return all dates between except the first" in {
  ...
  "Given two dates (with default bounds) and a step of 2 days, datesBetween" should "return every second date between" in {
    val start = daysOfMarch.head
    val end = daysOfMarch.last
